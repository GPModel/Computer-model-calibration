...
%         'ZFit=0;'
    ...
    [~,Sortidx]=sort(Obj_Fun_ZhQuantile(NewXhatPoints));%%%~+all previous+current design points +all previous xhatstarml			
    parfor id=1:90
    ...
    HistoryXhats=Xhats(InitialNumber:end,:);
...
%     [X,Y]=meshgrid(linspace(0,1,501));
%     [Z1,~,corrs]=Obj_Fun_MinusAEI1([X(:),Y(:)]);
%     Z2=Obj_Fun_MinusAEI1([X(:),Y(:)]);
    
%     figure()
%     mesh(X,Y,reshape(corrs,size(X)))
%     input('xxx')    
else
    Level1=1;
    ...
    [fval_MinusAEI1,Sortidx]=sort(Obj_Fun_MinusAEI1(NewAFPoints));
    parfor id=1:90
    ...
    [fval_MinusAEI2,Sortidx]=sort(Obj_Fun_MinusAEI2(NewAFPoints));
    parfor id=1:90
...
%%
function [fval_MinusAEI,EIs,Corrs,ZhCovs]=Fun_MinusAEI(TeD,MinZhRef,RatioCost,AEILevel,Dl,Respl,Dh,Resph,Sigmal,Thetal,Rho,Sigmah,Thetah,phi,ZNBC,Mu,invV,invVRes,nugget)
...
if AEILevel==2
    ...
    fvalAEIs(  [ZhCovs==0] |  (min(pdist2(TeD,Dh),[],2) < 10^(-3))         ) =0;   % 2 or 4  or 6  %n(TeD)*n(Dh) matrix
